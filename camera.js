            ...
            try {
                switch (d.f) {
                    ...
                    case 'onvif':
                        ...
                        d.IP_LIST.forEach((ip_entry, n) => {
                            d.PORT_LIST.forEach((port_entry, nn) => {
                                return new Cam({
                                    hostname: ip_entry,
                                    username: d.USERNAME,
                                    password: d.PASSWORD,
                                    ...
                                    let cam_obj = this;
                                    cam_obj.getSystemDateAndTime((er, date, xml) => {
                                        if (!er) err.date = date;
                                        cam_obj.getDeviceInformation((er, info, xml) => {
                                            if (!er) err.info = info;
                                            try {
...
app.get(['/:auth/mjpeg/:ke/:id', '/:auth/mjpeg/:ke/:id/:addon'], (req, res) => {
    if (req.params.addon == 'full') {
        ...
        s.auth(req.params, () => {
            ...
            let contentWriter, content = fs.readFileSync(config.defaultMjpeg, 'binary');
            res.write("--shinobi\r\n");
